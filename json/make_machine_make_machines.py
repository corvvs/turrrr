...
tape = ["1", ".", "+", "="]
first_read_transitions = [set_orderd_dict_list(t, 'A' + t + '_go_start', '*', 'LEFT') for t in tape]
...
for p in search_pattern:
    ...
    d['transitions'][p] += [set_orderd_dict_list('*', state, p[8], direction)]

for p in ['A', 'B']:
    state = 'read_' + p
    read_list = ['1', '.', '+', '=']
    d['transitions'][state] = [
        set_orderd_dict_list(r, p + r + '_go_start', '*', 'LEFT')
            for r in read_list
    ]
